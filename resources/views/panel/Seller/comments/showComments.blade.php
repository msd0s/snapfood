                        ...
                        <div class="table-responsive border-top userlist-table">
                            <table class="table card-table table-striped table-vcenter text-nowrap mb-0">
                                ...
                                <tr class="text-center">
                                    <th class="wd-lg-20p"><span>نام کاربر</span></th>
                                    <th class="wd-lg-20p"><span>امتیاز</span></th>
                                    <th class="wd-lg-20p"><span>نظر</span></th>
                                ...
                                @forelse($comments as $comment)
                                    <tr class="text-center">
                                        ...
                                        <td>
                                            {{ $comment['comment'] }}
                                            <form action="{{ route('seller.comment.update',$comment['id']) }}" method="post">
                                                @csrf
                                                @method('patch')
                                                <textarea name="answer" class="form-control" id="answer" rows="2">{{ $comment['answer'] }}</textarea>
                                                <button type="submit" class="btn btn-primary w-100 mt-1" name="send_answer">ثبت پاسخ</button>
                                            </form>
                                        </td>
                                        <td>
                                            <form action="{{ route('seller.comment.destroy',$comment['id']) }}" method="post">
                                                @csrf
                                                @method('delete')
                                                <button type="submit" name="delete_comment" class="btn btn-danger mb-2" @if($comment['delete_request']==1) disabled @endif><i class="las la-trash"></i>درخواست حذف</button>
                                            </form>
                                            <form action="{{ route('seller.comment.changestatus',$comment['id']) }}" method="post">
                                                @csrf
                                                @method('patch')
                                                <button type="submit" name="change_status" class="btn @if($comment['status']==0) btn-success @else btn-warning @endif mb-2" @if($comment['delete_request']==1) disabled @endif><i class="las la-refresh"></i>@if($comment['status']==0) فعال سازی @else غیرفعال سازی @endif</button>
                                            </form>
