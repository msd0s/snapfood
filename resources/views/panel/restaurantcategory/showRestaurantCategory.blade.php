...
@section('header')
<title>مدیریت دسته بندی رستوران</title>
...
@section('content')
                ...
                <div class="card">
                    <div class="card-header pb-0">
                        <div class="d-flex justify-content-between">
                            <h4 class="card-title mg-b-0">لیست دسته بندی ها</h4>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive border-top userlist-table">
                            <table class="table card-table table-striped table-vcenter text-nowrap mb-0">
                                ...
                                <tr>
                                    ...
                                    <th class="wd-lg-20p"><span>دسته بندی والد</span></th>
                                ...
                                @foreach($cats as $cat)
                                    <tr>
                                        <td>{{ $cat['title'] }}</td>
                                        <td>
                                            {!! catfind($cat['main_cat']) !!}
                                        </td>
                                        <td>
                                            <a href="{{ route('admin.restaurantcategory.edit',$cat['id']) }}" class="btn btn-sm btn-info">
                                                <i class="las la-pen"></i>
                                            </a>
                                            <form action="{{ route('admin.restaurantcategory.destroy',$cat['id']) }}" method="post" class="d-inline">
                                                @csrf
                                                @method('delete')
                        ...
                        {{--{{ $cats->appends(['sort' => 'pcatid'])->links() }}--}}
                    </div>
                </div>
            </div><!-- COL END -->
        </div>
    ...
    <?php
    function catfind($catid){
        if ($catid == 0) {
            return $category = 'شاخه اصلی';
        } else {
            $category = \App\Models\RestaurantCategory::find($catid);
        }
